...
In this Solution Pattern, you will continue to use the {create-messages-app} and the {retrieve-messages-app} applications from the _{sp1-title}_ Solution Pattern. You now need to protect them from unauthorised access by creating and enabling a single sign-on service.
