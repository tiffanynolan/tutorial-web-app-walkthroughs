...
. Go to the link:{sso-realm-url}[{realm-display-name}, window="_blank"], which is running on your {customer-sso-name} service. 
.. If prompted, enter the username `{shared-realm-username}` and password `password`.
. You will see the *Realm Settings* screen for the *{realm-name}* realm if the login is successful.
