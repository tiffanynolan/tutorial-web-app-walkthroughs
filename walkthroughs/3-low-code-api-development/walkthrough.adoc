...
[time=20]
== Creating an OpenAPI Spec using Apicurio
...
we'll be using Apicurio - a visual design tool - to generate this YAML/JSON
document.

=== Create the Base API Definition

. Navigate to the link:{apicurio-url}[Apicurio Console, window="_blank"]
...
. Drag and drop the OpenAPI Spec YAML file downloaded from Apicurio earlier into the *Upload an OpenAPI file* box and click the *Next* button.
. Click *Next* to navigate to the *Operations* screen.

[type=verification]
Is a *POST /greeting* operation displayed on the *Operations* screen in Fuse Online?

[type=verificationFail]
Verify that you created the POST operation correctly in the OpenAPI Spec using Apicurio. Contact an administrator if the problem persists.
