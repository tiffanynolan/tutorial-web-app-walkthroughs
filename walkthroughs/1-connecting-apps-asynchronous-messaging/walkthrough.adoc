...
. Click the *Create* button to start the *Create an instance* wizard.
. On the *Configure your component* form:
.. For the *Namespace*, select `{namespace-name}`.
...
.. Verify the *Address space plan* is set to *standard-small*.
.. Verify the *Authentication service* is set to *none-authservice*.
.. Click *Next*.
